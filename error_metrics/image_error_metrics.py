...
class ImageErrorMetrics:
    ...
    def _mse(self, estimated_srgb_image):
        return float(np.mean((self.ground_truth_srgb_image.astype(np.float32) - estimated_srgb_image.astype(np.float32)) ** 2))

    def _ciede_2000(self, estimated_srgb_image):
        gt_lab  = cv.cvtColor(self.ground_truth_srgb_image, cv.COLOR_BGR2LAB).astype(np.float32)
        est_lab = cv.cvtColor(estimated_srgb_image,         cv.COLOR_BGR2LAB).astype(np.float32)

        L1 = gt_lab[:, :, 0] * 100.0 / 255.0;  a1 = gt_lab[:, :, 1] - 128.0;  b1 = gt_lab[:, :, 2] - 128.0
        L2 = est_lab[:, :, 0] * 100.0 / 255.0; a2 = est_lab[:, :, 1] - 128.0; b2 = est_lab[:, :, 2] - 128.0
